...
class SudokuPuzzle(Puzzle):
    ...
    def fail_fast(self):
        """
        Return True if it is impossible to solve Sudoku i.e there is one open
        position that has no symbols from set available to put in it.
        ...
        # searching all available positions
        ...
        for index in list_:
            # creating set of all symbols used in row, column, and subsquare
            # where empty position available
            row_column = self._row_set(index).union(self._column_set(index))
            used_symbols = row_column.union(self._subsquare_set(index))

            # check whether all symbols are used
            used_symbols.remove("*")
            if used_symbols == self._symbol_set:
                return True
        return False
